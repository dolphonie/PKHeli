...
rem just compile the xrotor 40a code
SET BESCNO=186
...
%RaisonancePath%\Ride\bin\ma51.exe "BLHeli.asm" PRINT SET(BESCNO=%BESCNO%) OBJECT(Output\%BESCNAME%_%Revision%.OBJ) DEBUG EP QUIET PIN(%SilabsPath%\MCU\Inc;%RaisonancePath%\Ride\inc;%RaisonancePath%\Ride\inc\51) >> MakeHex_Result.txt
...
move BLHeli.LST Output\%BESC%.LST > nul
@ECHO. >> MakeHex_Result.txt
goto :eof
